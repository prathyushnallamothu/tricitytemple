    ...
    <b-sidebar id="sidebar-add-new-event" sidebar-class="sidebar-lg" :visible="isEventHandlerSidebarActive"
      ...
      <template #default="{ hide }">
        ...
        <div class="d-flex justify-content-between align-items-center content-sidebar-header px-2 py-1">
          <h5 class="mb-0">{{ event.id ? "Update" : "Add" }} Event</h5>
          <div>
            <feather-icon v-if="event.id" icon="TrashIcon" class="cursor-pointer" @click="
  $props.removeEventV2();
        ...
        <validation-observer #default="{ handleSubmit }" ref="refFormObserver">
          <!-- Form -->
          <b-form class="p-2" @submit.prevent="handleSubmit(submitFormV2)" @reset.prevent="resetForm">
            <!-- Title -->
            <validation-provider #default="validationContext" name="Title" rules="required">
              <b-form-group label="Title" label-for="event-title">
                <b-form-input id="event-title" v-model="event.title" autofocus
                  :state="getValidationState(validationContext)" trim placeholder="Event Title" />
            ...
            <validation-provider #default="validationContext" name="Start Date" rules="required">
              <b-form-group label="Start Date" label-for="start-date" :state="getValidationState(validationContext)">
                <flat-pickr v-model="event.start" class="form-control" :config="{
                  enableTime: true,
            ...
            <validation-provider #default="validationContext" name="End Date" rules="required">
              <b-form-group label="End Date" label-for="end-date" :state="getValidationState(validationContext)">
                <flat-pickr v-model="event.end" class="form-control" :config="endConfig" />
                <b-form-invalid-feedback :state="getValidationState(validationContext)">
                  {{ validationContext.errors[0] }}
                </b-form-invalid-feedback>
              </b-form-group>
            ...
            <b-form-group label="Location" label-for="event-location">
              <b-form-input id="event-location" v-model="event.location" trim placeholder="Event Location" />
            </b-form-group>

            <!-- Textarea -->
            <b-form-group label="Description" label-for="event-description">
          
                <quill-editor id="event-description" v-model="event.description" :options="snowOption" />
  
            </b-form-group>
            <b-form-group label="Image" label-for="Image">
              <b-form-file v-model="event.file" placeholder="Choose a file or drop it here..."
                drop-placeholder="Drop file here..." accept="image/*" />
              <img :src="event.img_url" height="128px" width="128px" alt="" srcset="" v-if="event.img_url != ''" />
              <b-card-text class="my-1">
                Selected file:
                <strong>{{ event.file ? event.file.name : "" }}</strong>
              </b-card-text>
            </b-form-group>
            <!-- Form Actions -->
  ...
  methods: {
    ...
    submitFormV2() {
      if (this.$props.event.id) {
        this.$props.updateEventV2();
      } else {
        this.$props.addEventV2();
      }
    },
  },
